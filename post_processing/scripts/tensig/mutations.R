...
getStrandOrientation <- function(vcf, orientation) {
    ...
    vcf_strand_orientation = strand(vcf)
    vcf_strand_orientation[queryHits(h)] = strand(orientation)[subjectHits(h)]
    vcf_strand_orientation[-queryHits(h)] = ORI[3]
    factor(vcf_strand_orientation, levels=ORI)
}
...
getNucleosomeState <- function(vcf, consensusState) {
  h <- findOverlaps(vcf, consensusState, ignore.strand=T)
  vcf_nuc_state = factor(strand(vcf), levels=c(0,1,2,3))
  vcf_nuc_state[-queryHits(h)] = 3
  vcf_nuc_state[queryHits(h)] = mcols(consensusState[subjectHits(h)])$state
  # stopifnot(length(h) == length(granges(vcf))) # sanityCheck
...
getChromatinState <- function(vcf, consensusState) {
    h <- findOverlaps(vcf, consensusState, ignore.strand=T)
    vcf_chrom_state = factor(strand(vcf), levels=CHROMATINSTATES)
    vcf_chrom_state[-queryHits(h)] = CHROMATINSTATES[16]
    vcf_chrom_state[queryHits(h)] = mcols(consensusState[subjectHits(h)])$state
    # stopifnot(length(h) == length(granges(vcf))) # sanityCheck
    # factor(mcols(consensusState[subjectHits(h)])$state, levels=CHROMATINSTATES)
    vcf_chrom_state
}
